...
【最重要:西班牙文的重音符號和 ñ 一定要完整保留】
西班牙文的重音字母(á é í ó ú)和 ñ、¿ ¡ 都必須正確寫出,絕對不可以省略或用普通字母代替。
例如:一定要寫 español(不是 espanol)、está(不是 esta)、año(不是 ano)、¿Cómo estás?(不是 Como estas?)。
重音和 ñ 會改變單字的意思和發音,對學習者非常重要,務必保留。LINE 完全能正常顯示這些字元。
...
重要:西班牙文的重音字母(á é í ó ú)、ñ、¿ ¡ 一定要完整正確寫出,絕對不可省略或用普通字母代替(例如要寫 está 不是 esta、español 不是 espanol、año 不是 ano)。
